      ...
      <VStack spacing={2} width="full">
        {players.map((player, index) => (
          <Box key={index} p={2} borderWidth="1px" borderRadius="lg">
            {player.name}
            {player.scores.reduce((total, score) => total + (score === "" ? 0 : score), 0) === Math.min(...players.map(p => p.scores.reduce((total, score) => total + (score === "" ? 0 : score), 0))) && <Icon as={FaTrophy} color="yellow.500" ml={2} />}
          </Box>
        ))}
      </VStack>
